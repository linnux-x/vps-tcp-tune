...
>
> 本页所有安装命令均指向本 fork（`linnux-x`）。**不要与上游的安装命令混用**——
> `bbr` 别名每次运行都会从其内置地址拉取脚本，混用会导致你以为在跑本 fork、实际跑的是上游版本。

## 本 fork 相对上游的改动

### 修复：菜单 17 `vless-all-in-one` 失效

原实现内联为 `wget -O vless-server.sh <url> && chmod +x && bash`，叠加四个问题导致「点了没反应」：裸用 `wget`（该命令仅由 Snell 安装流程附带安装，纯净 Debian 上通常没有，`&&` 链静默短路）、`needs_pause=false` 让错误一闪而过、下载到当前工作目录、完全不校验下载结果。

现改为复用脚本自带的 `safe_download_script()`，下载到临时目录，校验确为 shell 脚本，执行前显式确认。上游 GitLab 地址本身是活的，失效原因不在远端。

### 加固：严格模式

启用 `set -Eeuo pipefail`。原脚本 2.5 万行、420+ 函数、以 root 运行却无任何严格模式，任何命令失败都会被静默跳过继续操作系统——菜单 17 正是这样坏掉的。

同时加了 **ERR 陷阱**，失败时打印行号、所在函数与具体命令。没有它，`set -e` 只会让脚本突然消失，对这个体量几乎无法排查。

> **⚠️ 这会改变运行时行为。** 原本静默跳过的失败现在会中止脚本。若某个功能因此无法完成，可用
> `STRICT_MODE=0 bash net-tcp-tune.sh` 临时关闭，并把 ERR 陷阱打印的行号反馈到 Issue 以便修根因。
>
> **本项改动尚未在真实 Debian 环境逐功能验证，首次使用建议先在可丢弃的 VPS 上试跑。**

配套修复了 18 处会被 `set -e` 误杀的 `[ 条件 ] && 命令` 独立语句——其中主函数的配置加载若不修，全新 VPS 上脚本会在进入菜单前直接退出。

### 加固：下载校验

原 20 处下载动作仅 3 处涉及 sha256，其余只靠「文件非空」或 `unzip -t`，能挡截断损坏，挡不住内容被替换。新增两个校验入口：

| 助手 | 用途 | 已接入 |
|---|---|---|
| `verify_download()` | 已知版本的 SHA-256 严格校验；未固定的版本明确提示而非假装安全 | Snell 二进制 |
| `verify_binary_download()` | 文件魔数校验（ELF / gzip / zip），拦住 HTML 错误页被 `chmod +x` 装成可执行文件 | cloudflared、caddy |

**其余下载点刻意不固定哈希**：中国 IP 列表每日变化、speedtest 与版本探测本就是动态内容、`get.docker.com` 一类安装器永远指向最新版——钉死哈希会立刻失效，对它们类型校验才是正确手段。

顺带把 caddy 的下载路径从固定的 `/tmp/caddy.tar.gz` 改为 `mktemp`：该函数以 root 运行，写入可预测路径可被本地非特权用户预建符号链接劫持。

### 清理：ShellCheck SC2155

275 处 `local x=$(cmd)` 拆分为 `local x` + `x=$(cmd)`，warning 由 300 降至 25。

> **⚠️ 这同样改变运行时行为**：返回值不再被掩盖，配合 `set -e` 意味着原本静默失败的地方现在会中止。

剩余 25 条经逐条评估后保留：其中 14 条 `SC1083` 是 ShellCheck 不解析 nftables 语法导致的误报，5 条 `SC2207` 是有意用分词构造数组。

### 修正：fork 自引用

上游账号的自引用地址共 19 处改指本 fork（`bbr` 别名、README 下载地址、systemd `Documentation`）。别名识别正则改为同时接受两个账号，保证已安装上游版本的机器能被正确替换而非重复叠加。

changelog 中的原作者署名、star badge 与 star-history 链接仍指向上游，属致谢。
...
> **版本**: v5.4.8 🧪 **Snell v6 跟进官方 RC**：内核默认版本升至 **6.0.0rc**，菜单 12-8「Snell v6 Beta 测试专区」更名为「RC 测试专区」；版本检查新增 rc 识别，实例配置显式写入 `mode = default`（流量混淆 + AES 加密，可按需改 `unshaped` 提速约 10%）。⚠️ v6 仍是官方预发布，客户端需 Surge Mac Beta 渠道或 iOS TestFlight，建议仅用于测试节点。承接 v5.4.7 🐛 **修复 ARM64 机器执行功能 1 闪退**：原 ARM 分支依赖外部站点的第三方脚本，该站校验文件已失效导致必然失败，且错误提示被菜单清屏吞掉，表现为"一闪就回菜单"。经核实 BBR v3 至今未合入 Linux 主线内核，XanMod 官方也仅提供 x86-64 构建，**ARM 平台目前不存在官方 BBR v3 方案**；现功能 1 在 ARM 上会直接说明原因并引导使用**功能 3**（ARM 内核自带 BBR + fq，功能 3 原生可用、同样有效），同时移除了对外部域名脚本的下载执行。承接 v5.4.6：清理剩余 3 处可预测的临时文件路径（sing-box 安装 / 中国 IP 列表下载 / cloudflared 下载），全部改用 mktemp 随机路径并收紧权限，至此全脚本临时文件均不可被符号链接抢占。承接 v5.4.5：菜单32-7「OpenAI Responses API 转换代理」新增访问密钥鉴权——部署时自动生成 48 位访问密钥，客户端需在请求头携带 `Authorization: Bearer <访问密钥>` 才能使用代理，防止代理被他人白嫖你的上游 API Key；**已部署旧实例默认不受影响但仍无鉴权，建议进「修改配置」一路回车并重启，即可自动补发访问密钥启用保护**。同时收紧了代理实例配置文件的权限，并加固了 Xray 安装过程的临时文件安全。
...
- **🆕 v6 RC 测试专区 (菜单 12-8)**：独立部署 Snell v6.0.0rc，与 v5 **完全隔离**（独立二进制 `snell-server-v6`、服务 `snellv6-*.service`、配置目录 `/etc/snell-v6/`、保留端口文件），互不影响。含安装/卸载/查看配置/更新+修复/健康检查/检查更新；安装时会先自检 v6 二进制，若官方包缺运行库，会自动或提示安装兼容依赖。配置默认写入 `mode = default`（流量混淆 + AES 加密，v6 的抗指纹特性），可按需改为 `unshaped`（关混淆仅加密，约快 10%）。⚠️ v6 仍是官方预发布(RC)，客户端需 Surge Mac Beta 渠道或 iOS TestFlight，App Store 正式版无法连接，建议仅用于测试节点
...
**Q: 我的 VPS 是 ARM64（aarch64），能装 BBR v3 吗？**

A: 目前不能，这不是脚本的限制。BBR v3 至今未合入 Linux 主线内核，必须使用打过补丁重新编译的内核，而 XanMod 官方只提供 x86-64 构建，ARM 平台没有官方方案。**但 ARM 机器请直接使用功能 3**：ARM 内核自带 BBR + fq 队列算法，功能 3 的带宽检测与缓冲区调优在 ARM 上原生可用，同样能获得明显提升。功能 1 在 ARM 上会直接提示这一点。
